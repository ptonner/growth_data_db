...
def create(id):
    ...
    if form.validate_on_submit():
        ...
        if form.covariates.data:
            covariates = [Covariate(designs=[d]) for d in phenotype.designs if d.name in form.covariates.data]

            for c in form.covariates.data:
                if ':' in c:
                    designs = c.split(":")
                    covariates.append(Covariate(designs=[d for d in phenotype.designs if d.name in designs]))
        else:
            covariates = []
...
def covariate(id):
    searchform = SearchForm()
    covariate = machine.session.query(Covariate).filter_by(id=id).one_or_none()
...
def create_test(id):
    ...
    if form.validate_on_submit():
        ...
        am = machine.session.query(Model).filter_by(id=form.alternative_model.data).one_or_none()
        nm = machine.session.query(Model).filter_by(id=form.null_model.data).one_or_none()

        test = Test(alternative_model=am, null_model=nm, phenotype=phenotype)
        machine.session.add(test)
        machine.session.commit()
...
def delete_test(id):
    searchform = SearchForm()
    test = machine.session.query(Test).filter_by(id=id).one_or_none()
